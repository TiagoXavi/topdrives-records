<template>
  <div class="Main_DarkScroll Main_DarkScrollMini BaseFilterDescription_Root">
...
export default {
  ...
  computed: {
    ...
    descResolved() {
      ...
      if (f.yearModel && JSON.stringify(f.yearModel) !== '[1910,2026]') result.push({ label: this.$tc("c_year", 2), value: f.yearModel.join("-") });
      if (f.rqModel && JSON.stringify(f.rqModel) !== '[10,120]') result.push({ label: "RQ", value: f.rqModel.join("-") });
      if (f.seatsModel && JSON.stringify(f.seatsModel) !== '[1,9]') result.push({ label: this.$t("c_seats"), value: f.seatsModel.join("-") });

      if (f.topSpeedModel && JSON.stringify(f.topSpeedModel) !== '[25,375]') result.push({ label: this.$t("c_topSpeed"), value: f.topSpeedModel.join("-") });
      if (f.acelModel && JSON.stringify(f.acelModel) !== '[1.4,40]') result.push({ label: "0-60", value: f.acelModel.join("-") });
