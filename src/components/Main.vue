<template>
  <div
    ...
    :style="mode !== 'compare' ? `--cell-width: ${windowWidth < 1200 ? 115 : 230}px;` : ''"
    class="Main_Layout"
        ...
        <div v-if="showCarsFix" class="Main_CarList" @click.stop="mainCarsListGeneralClick($event)" @mouseleave="hoverIndex = -1;">
          <template v-for="(car, carIx) in carDetailsList">
            <Car
              ...
              @cog="openCarTuneDialog($event)"
              @changed="changedTune($event)" />
          </template>
    ...
    <div
      ...
      <div v-else-if="cgRound.date" class="Cg_Mid">
        ...
        <div>{{ "isRoundEmptyForModders" }} {{ isRoundEmptyForModders }}</div>
        ...
        <template v-else-if="!isRoundComplete || (user && !user.mod && isRoundComplete && isRoundReadyForSaveUser)">
          ...
          <template v-else>
            ...
            <!-- CG user mod -->
            <template v-if="false && isRoundEmptyForModders && !isRoundComplete && !cgRound.reservedTo">
        ...
        <template v-if="!cgNewSubmitByMod && (isRoundComplete || user) && (!cgRound.reservedTo || (user && cgRound.reservedTo === user.username))">
          <div
            :class="{ Cg_YouBankHideLosers: cgHideLosersInSolutions }"
            class="Cg_Box"
            ...
            <div
              ...
              <div class="Cg_Opponent">
                <BaseCard
                  v-if="race.rid"
                  :car="Vue.all_carsObj[race.rid]"
                  :selectedTune="race.tune"
                  :fix-back="true"
                  :downloadLoading="Vue.utils.cacheLoading && Vue.utils.rnsDownloading.includes(Vue.rn_to_rid.indexOf(race.rid))"
                  :needSave="needSave"
                  :draggable="false"
                  :cgOppo="true"
                  :count="count"
                  ...
                  @cog="cgShowTuneDialog(race, true, irace)"
                  @longTouch="cgShowTuneDialog(race, true, irace)"
                  @delete="race.rid = null; race.tune = null; calcRaceResult(race);"
                  @refreshTune="cgChangeTuneOppo(undefined, race)" />
                ...
                <div v-if="race.rid && !race.tune" class="Cg_OppoTuneBox">
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('332', race)">332</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('323', race)">323</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('233', race)">233</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('222', race)">222</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('111', race)">111</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgChangeTuneOppo('000', race)">000</button>
                  <button
                    class="D_Button Cg_OppoTuneButton"
                    @click="cgDialogCustomOppoTune(race)">{{ $t("m_other") }}</button>
                </div>
              </div>
              <div class="Cg_Track">
                <Row
              ...
              <div v-if="race.rid && race.tune && race.track" class="Cg_ThemTime">
                <BaseTimeCell
                  :rid="race.rid"
                  :selectedTune="race.tune"
                  :track="race.track"
              ...
              <template v-if="race.track && race.rid && (cgRound.lastAnalyze || (!!user && user.mod) || cgRound.isPreview) && (!cgIsApproving || cgRound.isPreview)">
                <template v-if="!cgRound.isPreview">
                  <div
                    :class="{
                      Cg_PointsRed: (race.cars[race.carIndex] || {}).points < 0 && race.track && race.rid,
                      Cg_PointsGreen: (race.cars[race.carIndex] || {}).points > 0 && race.track && race.rid,
                      Cg_PointsGrey: (race.cars[race.carIndex] || {}).points === 0 && race.track && race.rid,
                    }"
                    class="Cg_Divider"
                  ...
                  <div
                    v-if="(race.cars[race.carIndex] || {}).rid && race.track"
                    class="Cg_YouTime">
                    ...
                    />
                    <BaseTimeCell
                      :rid="race.cars[race.carIndex].rid"
                      :selectedTune="race.cars[race.carIndex].selectedTune"
                      :track="race.track"
                      :count="count"
                    />
                    <!-- <Row
                      :car="(race.cars[race.carIndex] || {}).rid"
                      :list="race.resolvedTracks"
                      :loggedin="!!user"
                      :user="user"
                      :voteLoading="voteLoading"
                      ...
                      @showTuneDialog="cgShowTuneDialog(race.cars[race.carIndex].car, race, false, irace)"
                    /> -->
                  </div>
                ...
                <div class="Cg_YouBank">
                  <div :class="{ Cg_YouBankBoxPreview: cgRound.isPreview }" class="Cg_YouBankBox">
                    ...
                    <template v-for="(bankCar, index) in race.cars">
                      ...
                      <button
                        v-if="bankCar.points !== $t('m_notime') && bankCar.selectedTune"
                        :disabled="cgLoadingAny"
                        :key="index"
                        ...
                        <div class="Cg_BankTuneNew">{{ bankCar.selectedTune }}</div>
          ...
          <div v-if="user && user.mod && user.username === 'TiagoXavi' && forceShowAnalyse" class="Cg_BottomModTools" style="margin-top: 30px; width: 70%; margin-left: auto; margin-right: auto;">
            <textarea
              ...
              placeholder="Challenge raw"
            />
            <div style="text-align: center;">
              <button
                :class="{ D_Button_Loading: cgSaveLoading }"
                class="D_Button D_ButtonDark D_ButtonDark2 D_ButtonGreen"
                @click="cgPrepareSaveRaw()">{{ $t("m_send") }}</button>
            </div>
          </div>
      ...
      <div v-else-if="cgCurrentRound === 'd' && cg.rounds?.['d']" class="Cg_Mid">
        <!-- DDASH -->
        <!-- {{ cgDashShowSolutionsCount-1 }} {{ typeof cgDashShowSolutionsCount-1 }} -->
        ...
        <div class="Cg_DashHeader">
          <div class="Cg_DashHeaderControls">
            ...
            <BaseSwitch
              :value="!cgDashShowOpponents && !cgDashShowTrackset && !cgDashShowOpponentsTimes && cgDashCompact"
              :disabled="cgDashShowOpponents || cgDashShowTrackset || cgDashShowOpponentsTimes"
              :label="$t('m_compact')"
              :horizontal="false"
              name="cgDashCompact"
        ...
        <BaseExpandDiv :active="cgDashListGarageUpgrade.length > 0 && Vue.garageObj.loaded && cgDashUseMyGarage" class="Cg_DashUpgradesSimBox">
          <div class="BaseCarList_SpecificCar" style="padding-top: 15px;">
            <div class="Cg_DashUpgradesTitle">{{ $t("m_upgradesSimulation") }}</div>
            <div class="BaseCarList_CarsList">
        ...
        <div class="Cg_DashReloadHeader D_Center2" style="margin-top: 15px;">
          <button
            :disabled="cgDashLoaded"
            class="D_Button D_ButtonDark"
            @click="cgInitDashDefault()">
        ...
        <BaseExpandDiv :active="Vue.garageObj.loaded && cgDashUseMyGarage && cg.rounds['d'].problemRounds.length > 0">
          <div class="Cg_DashReport" style="padding-top: 15px;">
            <div class="Cg_DashReportProblems BaseCarList_CarsList" style="align-items: center;">
              <div class="Cg_DashReportProblemsTitle">
                <i class="ticon-close_3 Cg_DashProblemIcon" aria-hidden="true"/>
                <span>{{ $t("m_faults") }}: </span>
              </div>
              {{ cg.rounds['d'].problemRounds.map(x => x+1).join(', ') }}
            </div>
          </div>
        </BaseExpandDiv>

        <div class="Cg_DashBody" :class="{ Cg_DashCompact: cgDashCompactResult }">
          ...
          <RecycleScroller
            ...
            <template v-slot="{ item, index, active }">
              <div
                v-if="cg.rounds[index]"
                :class="{
                  Cg_DashItemOverRq: (cgDashShowBestSolution ? item.bestSolutionRqSum : item.solutionsRqSum) > cg.rounds[index].rqLimit,
                  Cg_DashHasProblem: cg.rounds['d'].problemRounds.includes(index),
                }"
                class="Cg_DashItem">

                <div class="Cg_DashItemHeader" :style="`--width: ${windowWidth < 1200 ? 115*5 : 230*5}px;`">
                  <button class="D_Button Cg_DashItemRound" @click="loadCgRound(cgCurrentId, index)">
                    <span>{{ $tc("m_round", 1) }} {{ index+1+cgCurrentRoundSum }}</span>
                    <i v-if="cg.rounds[index].lastAnalyze" class="ticon-star Main_RoundDoneIcon" aria-hidden="true"/>
                    <i v-else-if="cg.rounds[index].toApprove && cg.rounds[index].toApprove.length > 0" class="ticon-star Main_RoundDoneIcon" style="color: unset;" aria-hidden="true"/>
                ...
                <BaseExpandDiv :active="cgDashShowOpponents" class="Cg_DashItemOppos">
                  ...
                  <div class="Cg_DashItemOpposMid">
                    <BaseCarsTeam
                      ...
                      style="--gap: 4px;"
                      prefix="Dash_Oppo"
                      @cog="cgDashcogClick(item.oppos[$event], $event, index)"
                    />
                  </div>
                </BaseExpandDiv>
                ...
                <template v-if="cg.rounds[index].lastAnalyze">
                  ...
                  <BaseExpandDiv :active="cgDashShowBestSolution" class="Cg_DashBestSolution">
                    <!-- Best solution -->
                    <div v-if="!cgDashCompactResult" class="Cg_DashItemOpposMid">
                      <BaseCarsTeam
                        :cars="item.bestSolution"
                        :filterToImport="{}"
                        :width="226"
                        ...
                        style="--gap: 4px;"
                        prefix="Dash_Best"
                        @cog="cgDashcogClick(item.bestSolution[$event], $event, index)"
                        @delete="cgDashSolutionClick(null, index, $event)"
                      />
                    </div>
                    <div v-else class="Cg_DashBestSolutionCompact">
                      <template v-for="(car, icar) in item.bestSolution">
                        <button
                          :key="`${icar}_${car.rid}`"
                          class="D_Button D_ButtonDark D_ButtonDark2 Cg_BankButton"
                          @click="cgDashcogClick(item.bestSolution[icar], icar, index)">
                          <BaseCardMini
                            :car="Vue.all_carsObj[car.rid]"
                            :tuneText="car.selectedTune"
                          />
                        </button>
                      </template>
                    </div>
                  ...
                  <div class="Cg_DashItemSolutionsLayout">
                    <div
                      ...
                      <button
                        v-if="item.solutions[irace].length > cgDashShowSolutionsCount-1"
                        class="D_Button D_ButtonLink Row_ShowMoreButton Cg_DashShowMoreButton"
                        @click="cgDashViewAllSolutions(index, irace)">
                        +{{ item.solutions[irace].length - (cgDashShowSolutionsCount-1) }}<span class="Cg_DashShowMoreLabel">&nbsp;{{ $tc("m_solution", item.solutions[irace].length - (cgDashShowSolutionsCount-1)) }}</span>
                      </button>
      ...
      <div class="Cg_Mid"> <!-- EVENT -->
        <template v-if="eventCurrentId">
          ...
          <template v-if="eventTab === 'trackset'">
            ...
            <div v-if="user && user.mod && user.username === 'TiagoXavi' && !eventBlockAddTrackset" class="Cg_BottomModTools" style="margin-top: 30px; width: 70%; margin-left: auto; margin-right: auto;">
              <textarea
                v-model="eventRaw"
                rows="15"
                class="Main_TextArea data-hj-allow"
                placeholder="Event raw"
              />
              <div style="text-align: center;">
                <button
                  :class="{ D_Button_Loading: clubLoadingAny }"
                  class="D_Button D_ButtonDark D_ButtonDark2 D_ButtonGreen"
                  @click="eventPrepareSaveRaw()">{{ $t("m_send") }}</button>
              </div>
            </div>
    ...
    <BaseDialog
      ...
      :transparent="false"
      :class="mode !== 'compare' ? `BaseDialog_RacePos${tuneDialogCarIndex}` : ''"
      max-width="420px"
      min-width="240px"
      @close="closeTune()">
      <div class="Main_TuneDialog ">
        <div v-if="Vue.utils.R_MedalsLoaded && tuneDialogCarRid && Vue.all_carsObj[tuneDialogCarRid].R_Medals_score !== undefined" class="Main_ShortCutBox">
          <button class="D_Button Main_ShortCutBoxButton" @click="openCarFullDetail(tuneDialogCarConfig)">
            <img
              src="/assets/fire100.png"
              class="Main_ShortCutBoxFire"
            />
            <div class="Main_ShortCutBoxValue">{{ Vue.all_carsObj[tuneDialogCarRid].prize && Vue.all_carsObj[tuneDialogCarRid].R_Medals_score === 0 ? '?' : Vue.all_carsObj[tuneDialogCarRid].R_Medals_score }}</div>
        ...
        <div v-if="tuneDialogActive" class="Row_DialogLayout">
          <div
            ...
            <div class="Row_OrderBoxLayout">
              <button
                v-if="mode === 'compare' && tuneDialogCarIndex > -1"
          ...
          <div v-if="!this.tuneDialogisOppo" class="Row_DialogHeader">
            <button
              ...
              :class="{
                Row_DialogButtonTuneActive: tuneDialogCarConfig.selectedTune === item,
                Row_DialogButtonTuneWin: tuneDialogTuneWins.includes(item),
                Row_DialogButtonTuneLose: tuneDialogTuneLose.includes(item)
              }"
            ...
            <button
              ...
              class="D_Button Row_DialogButtonTune"
              @click="chooseCustomTune(tuneDialogCarConfig)">
          ...
          <div v-if="
            !tuneDialogisOppo && 
            (
              Vue.all_carsObj[tuneDialogCarRid].class === 'S' ||
              Vue.all_carsObj[tuneDialogCarRid].class === 'A' || 
              Vue.all_carsObj[tuneDialogCarRid].class === 'B'
            )
          " class="Row_DialogHeader">
            <button
          ...
          <div class="Row_DialogBody Space_TopPlus">
            <div class="Row_DialogCard">
              <div class="Row_DialogCardLeft">
                <BaseCard
                  :car="Vue.all_carsObj[tuneDialogCarRid]"
                  :selectedTune="tuneDialogCarConfig.selectedTune"
                  :isDialogBox="true"
                  :count="count"
                  :options="false" />
              </div>
              <div v-if="(tuneDialogCarConfig.selectedTune && !tuneDialogCarConfig.selectedTune.includes('Other')) && tuneDialogCarConfig.selectedTune !== '000'" class="Row_DialogCardRight">
                <BaseText
                  :value="Vue.resolveStat(tuneDialogCarRid, 'topSpeed', tuneDialogCarConfig.selectedTune)"
                  :disabled="true"
                  type="topSpeed"
                  :label="$t('c_topSpeed')"
                  class="Space_Bottom Row_FieldStat"
                  placeholder="-"
                  @change="changeStatCar(tuneDialogCarConfig, 'topSpeed', $event)" />
                <BaseText
                  :value="Vue.resolveStat(tuneDialogCarRid, 'acel', tuneDialogCarConfig.selectedTune)"
                  :disabled="true"
                  type="acel"
                  label="0-60mph"
                  class="Space_Bottom Row_FieldStat"
                  placeholder="-"
                  @change="changeStatCar(tuneDialogCarConfig, 'acel', $event)" />
                <BaseText
                  :value="Vue.resolveStat(tuneDialogCarRid, 'hand', tuneDialogCarConfig.selectedTune)"
                  :disabled="true"
                  type="hand"
                  :label="$t('c_handling')"
                  class="Row_FieldStat"
                  placeholder="-"
                  @change="changeStatCar(tuneDialogCarConfig, 'hand', $event)" />
              </div>
              <div v-else class="Row_DialogCardRight">
                <BaseText :value="Vue.resolveStat(tuneDialogCarRid, 'topSpeed')" :label="$t('c_topSpeed')" class="Space_Bottom Row_FieldStat" :disabled="true" />
                <BaseText :value="Vue.resolveStat(tuneDialogCarRid, 'acel')" label="0-60mph" class="Space_Bottom Row_FieldStat" :disabled="true" />
                <BaseText :value="Vue.resolveStat(tuneDialogCarRid, 'hand')" :label="$t('c_handling')" class="Row_FieldStat" :disabled="true" />
              </div>
            </div>
            <div v-if="mode === 'compare' && Vue.utils.statistics[`car_${tuneDialogCarRid}`]" class="Main_ViewsBox">
              <div class="Main_ViewsCountDialog">{{ Vue.utils.statistics[`car_${tuneDialogCarRid}`].c }} views</div>
            </div>
          </div>
          <div v-if="Vue.all_carsObj[tuneDialogCarRid].tags && Vue.all_carsObj[tuneDialogCarRid].tags.length > 0" class="Row_DialogCardTags" style="margin-top: 12px;">
            <BaseGameTag
              v-for="tag in Vue.all_carsObj[tuneDialogCarRid].tags"
              :key="tag"
              :tag="tag" />
          </div>
          <div class="Row_DialogCardDual Space_TopPlus">
            <BaseCarStats :car="Vue.all_carsObj[tuneDialogCarRid]" @mraChanged="carDetailsList.map(x => {
              if (x.rid === tuneDialogCarRid) {
                Vue.set(x, 'mra', $event);
              }
            })" />
          </div>
    ...
    <BaseDialog
      ...
      :transparent="true"
      :lazy="true"
      :class="`BaseDialog_RacePos${cgDashSolutionsDialogRace}`"
      max-width="380px"
      min-width="240px"
...
export default {
  ...
  data() {
    return {
      ...
      tuneDialogCarRid: null,
      tuneDialogCarConfig: null, // selected tune, TCode
  ...
  beforeMount() {
    ...
    if (cgDashUseMyGarage) {
      cgDashUseMyGarage = JSON.parse(cgDashUseMyGarage);
      this.cgDashUseMyGarage = cgDashUseMyGarage;
    }
  ...
  activated() {
    ...
    vm.unsubscribe = vm.$store.subscribe(mutation => {
      ...
      if (mutation.type == "SHOW_TUNE") {
        ...
        if (car) {
          vm.tuneDialogCarRid = car.rid;
          vm.tuneDialogCarConfig = car;
  ...
  computed: {
    ...
    maxCarNumber() {
      if (this.currentTracks.length < 2) return 30;
      return 15;
    },
    ...
    highlights() {
      ...
      let trackCodes = this.currentTracks.map(x => `${x.id}_a${x.surface}${x.cond}`);

      // insert every type of track in both result && sortedByTracks
      trackCodes.forEach(trCode => {
        vm.countTimesPerTrack[trCode] = 0;
        sortedByTracks[trCode] = [];
        result.map(y => {
          y[trCode] = null;
        })
      });
      
      // insere os times de cada carro em suas respectivas trCode
      let val;
      this.carDetailsList.map(car => {
        if ( car.selectedTune ) {
          trackCodes.forEach(trCode => {
            val = Vue.getTimeFromCache(car.rid, car.selectedTune, trCode);
            if (val) {
              sortedByTracks[trCode].push(Vue.getTimeFromCache(car.rid, car.selectedTune, trCode));
              vm.countTimesPerTrack[trCode] = vm.countTimesPerTrack[trCode] + 1;
            }
          });
        }
      });
      ...
      Object.keys( sortedByTracks ).forEach(function (trCode) {
        if (sortedByTracks[trCode].length === 1) {
          sortedByTracks[trCode] = [];
        } else {
          sortedByTracks[trCode].sort(function(a, b) {
            if (trCode.includes('testBowl')) return b - a;
            if (a === 0 || a === null) return 9999999;
            if (b === 0 || b === null) return -9999999;
            if (a < 0 && b > 0) return 9999999;
            if (a > 0 && b < 0) return -9999999;
            return a - b;
          });
          sortedByTracks[trCode] = [...new Set(sortedByTracks[trCode])];
        }
      });
      ...
      let tempValue;
      result.map((x, ix) => {
        let rid = vm.carDetailsList[ix].rid;
        Object.keys( x ).forEach(function (trCode) {
          if (Vue.getTimeFromCache(rid, vm.carDetailsList[ix].selectedTune, trCode) !== undefined) {
            tempValue = Vue.getTimeFromCache(rid, vm.carDetailsList[ix].selectedTune, trCode);
            x[trCode] = sortedByTracks[trCode].indexOf(tempValue);
          }
        });
      })
    ...
    showAnalyse() {
      ...
      this.cgRound.races.find(race => {
        ...
        if (race.cars && race.cars.length > 4) {
          show = false;
          return true;
        }
      })
    ...
    cgDashItemHeight() {
      ...
      // if (this.cgDashCompactResult) sum -= 40;
      return sum;
    },
    cgSolutionsHeight() {
      if (this.mode !== 'challenges') return 0;
      let sum = 35;
      if (this.cgDashShowSolutionsCount-1) sum += 10; // padding
      sum += 42 * (this.cgDashShowSolutionsCount-1);
      if (this.cgDashCompactResult) sum += 42;
      else sum += 140;
      if (this.cgDashShowOpponentsTimes) sum += (35 + 50);
      if (this.cgDashCompactResult) sum -= 5;
      return sum;
    },
    cgDashCompactResult() {
      if (this.cgDashShowOpponents) return false;
      if (this.cgDashShowTrackset) return false;
      if (this.cgDashShowOpponentsTimes) return false;
      return this.cgDashCompact;
    }
  },
  methods: {
    ...
    deleteCar(index) {
      if (this.mode === 'challenges') {
        // only via dialog
        if (this.cgCurrentRound === 'd') {
          this.cgDashSolutionClick(null, this.cgDashSolutionsDialogRound, this.tuneDialogCarIndex);
          this.tuneDialogActive = false;
          return;
        } else if (this.tuneDialogisOppo) {
    ...
    addCar(newCar) {
      if (this.carDetailsList.length < this.maxCarNumber) {
        newCar.softId = this.nextId;
        newCar.TCode = Vue.getTCod(newCar.rid, newCar.selectedTune);
        newCar.class = Vue.all_carsObj[newCar.rid].class;
        newCar.rq = Vue.all_carsObj[newCar.rid].rq;
        newCar.rn = Vue.rn_to_rid.indexOf(newCar.rid);
    ...
    prepareCars(cars) {
      ...
      if (cars && cars.length > 0) {
        ...
        cars.map(y => {
          ...
          if (found) {
            this.addCar({ rid: y.rid, selectedTune: y.selectedTune });
          }
          // } else {
          //   result.push({
          //     rid: y.rid,
    ...
    generateCarsList() {
      ...
      let list = [];
      this.carDetailsList.map(x => {
        list.push(`RQ${x.rq} ${Vue.all_carsObj[x.rid].name}`)
      });
      list = [...new Set(list)];
    ...
    resolveChangeTime(car, NEW, number, tune) {
      ...
      delete car.data[tune].times[track].isTimePredicted;
    ...
    changeTuneCar(car, newTune, fromRow = false) {
      ...
      if (this.mode === 'challenges' && this.tuneDialogActive) {
        ...
        Vue.set(car, "TCode", Vue.getTCod(car.rid, newTune));
        if (!fromRow) {
          // this.showCarsFix = false;
          // this.$nextTick().then(() => {
          //   this.showCarsFix = true;
    ...
    loadChallenges(resolveInitial = true) {
      ...
      .then(res => {
        ...
        if ((this.user && this.user.mod) || this.usedToMod || import.meta.env.DEV) {
          this.cgList.push({
      ...
      .then(res => {
        ...
        if (cg.date === res.data.date) {
          ...
          if (this.user && this.user.mod) {
            ...
            this.cgRaw = JSON.stringify(this.cgRaw, null, 2);
          }
        }
        this.cgResolveIcons();
    ...
    cgDialogCustomOppoTune(race) {
      let vm = this;

      let action = function() {
        vm.cgChangeTuneOppo(vm.$store.state.confirmDialog.advanced.text.trim(), race);
    ...
    cgChangeTuneOppo(tune, race) {
      Vue.set(race, "tune", tune);
    ...
    cgChangeTuneYou(race, newTune) {
      if (typeof newTune === "object") {
        newTune = newTune.selectedTune;
      }
      ...
      if (found > -1) {
        ...
        race.cars[race.carIndex].selectedTune = newTune;
        race.cars[race.carIndex].points = this.cgTryGetPointsFromCache(race);
    ...
    cgTryGetPointsFromCache(race) {
      ...
      let youTune = race.cars[race.carIndex].tune;
      let points = undefined;
      let time = Vue.getTimeFromCache(youRid, youTune, race.track);
      if (time === undefined) return points;
      points = (Vue.userPoints(time, race.time, race.track) || { v: 1000 }).v;
      return points;
    },
    cgShowTuneDialog(race, isOppo = false, irace) {
      if (Vue.utils.cacheLoading) return;

      this.tuneDialogCarRid = race.rid;
      race.selectedTune = race.tune;
      this.tuneDialogCarConfig = race;
      if (!isOppo) {
        this.tuneDialogCarRid = race.cars[race.carIndex].rid;
        this.tuneDialogCarConfig = race.cars[race.carIndex];
      }
      this.tuneDialogCarIndex = irace;
      this.tuneDialogisOppo = isOppo;
      this.tuneDialogActive = true;
      this.tuneDialogTuneWins = [];
      this.tuneDialogTuneLose = [];
      if (!isOppo) {
        this.cgLoadYouTimesTuneAll(race, irace);
      }
      this.tuneDialogRace = race;
      Vue.carsCompile("R_Medals");
    },
    cgLoadYouTimesTuneAll(race, irace) {
      let tunes = ["332", "323", "233", "111", "000"];
      if (
        Vue.all_carsObj[this.tuneDialogCarRid].class === 'S' ||
        Vue.all_carsObj[this.tuneDialogCarRid].class === 'A' || 
        Vue.all_carsObj[this.tuneDialogCarRid].class === 'B'
      ) {
        tunes = [
          ...tunes,
          ...Vue.extraTunes
      ...
      tunes.forEach(tune => {
        let time = Vue.timeCell(this.tuneDialogCarRid, tune, race.track);
        if (time === null || time === "!loading") return;
        if (Vue.timeIsBest(time, race.time, race.track)) {
          this.tuneDialogTuneWins.push(tune);
        } else {
          this.tuneDialogTuneLose.push(tune);
        }
      });
    },
    cgLoadYouTimesAfter() {
      this.cgLoadYouTimesTuneAll(this.tuneDialogRace, this.tuneDialogCarIndex);
    },
    calcRaceResult(race, isCgInitial) {
      ...
      // if (!(race.cars[race.carIndex] || {}).rid || !((race.cars[race.carIndex] || {}).tune)) {
    ...
    loadPrevRound() {
      ...
      if (this.cgCurrentRound === 0) result = "d";
    ...
    loadNextRound() {
      ...
      this.forceShowAnalyse = false;
      let result = this.cgCurrentRound+1;
      if (this.cgCurrentRound === "d") result = 0;
      this.loadCgRound(this.cgCurrentId, result)
    },
    cgOpenNewCg() {
    ...
    cgOpenPointsEdit(race) {
      ...
      let car = Vue.all_carsObj[race.cars[race.carIndex].rid];
      let trytime
      if (!car) return;
      try {
    ...
    cgAnalyseRoundFinish() {
      let clearRound = this.cgRound.races.map(race => {
        return {
          track: race.track,
          rid: race.rid,
          tune: race.tune,
    ...
    cgSubmitCompleteJson(createNew) {
      let json = JSON.parse(this.cgCompleteJson);
      let oppos = json.ladder.opponents;
      // let list = oppos.map(x => x.cardId);
      // list = [...new Set(list)];
      let newCg;
      newCg = JSON.parse(JSON.stringify(this.cg));
      if (createNew) {
        newCg = {
          date: null,
          rounds: [],
          name: json.ladder.name,
          // name: "Proving Grounds: test2"
        }
        json.ladder.rungPrizes.map((rung, irung) => {
          let races = [];
          [...Array(5)].map((_, i) => {
            races.push({
              "cars": [],
      ...
      if (this.cgCurrentRoundSum && !createNew) {
      ...
      axios.post(url, newCg)
      .then(res => {
        this.cgNewDialog = false;
        if (createNew) {
          this.cgCurrentId = res.data.date;
          this.cgCurrentRound = 0;
        }
        this.loadChallenges();
        this.cgCompleteJson = "";
        this.forceShowAnalyse = false;
    ...
    cgInitDash(useGarage) {
      ...
      setTimeout(() => {
        this.cgDashLoaded = false;
        this.cgReloadIndex = -1;
      }, 1000);
    },
    cgResolveBestSolution(_r, round, useGarage, iround, isRerun = false) {
      let usedRids = {};
      let usedHids = {};
      let rqSum = 0;
      ...
      // fix no car
      let count = 0;
      let noCarIndex = _r.bestSolution.findIndex(x => !x || !x.rid);
      ...
      while (noCarIndex > -1 && count < 15) {
        ...
        if (res) {
          res = this.cgTryReplacer(round, _r, usedRids, usedHids, noCarIndex, useGarage, 0, iround, true, isRerun);
        }
        noCarIndex = _r.bestSolution.findIndex(x => !x || !x.rid);
        currIndex++;
        if (currIndex >= _r.bestSolution.length) currIndex = 0;
        count++;
      }

      // console.log( _r.bestSolution.map(x => {
      //   return {
      //     rid: x?.rid,
      ...
      rqSum = this.cgSumRQ(_r.bestSolution);
      if (round.rqLimit && rqSum > round.rqLimit) {
        let rqList = _r.bestSolution.map((car, icar) => {
        ...
        for (let i = 0; i < rqList.length; i++) {
          let icar = rqList[i][0];
          let car = _r.bestSolution[icar];
          if (!car || !car.rid) continue;
          
          this.cgTryReplacer(round, _r, usedRids, usedHids, icar, useGarage, rqSum, iround, false, isRerun, true);
          rqSum = this.cgSumRQ(_r.bestSolution);
          if (rqSum <= round.rqLimit) break;
        }
      }
      if (round.rqLimit && rqSum > round.rqLimit ) { // consecutive replacer technique
        let rqList = _r.bestSolution.map((car, icar) => {
          if (!car || !car.rid) return [icar, 0];
          return [icar, Vue.all_carsObj[car.rid]?.rq || 0];
        });
        ...
        count = 0;
        currIndex = rqList[count % 5][0];
        while ((round.rqLimit && this.cgSumRQ(_r.bestSolution) > round.rqLimit) && count < 15) {
          // if (iround === 0 && useGarage && currIndex === 1) {
          //   debugger;
          // }
          this.cgTryReplacer(round, _r, usedRids, usedHids, currIndex, useGarage, 0, iround, true, isRerun, true);
          // console.log(_r.bestSolution.map(x => `${Vue.all_carsObj[x?.rid]?.rq || 0} ${x?.rid}`));
          count++;
          currIndex = rqList[count % 5][0];
        }
      }
      ...
      if (isRerun) {
        _r.solutions.map((listCars, irace) => {
          _r.solutions[irace] = _r.solutions[irace].filter(car => {
            if (_r.bestSolution[irace] && car.rid === _r.bestSolution[irace].rid && car.tune === _r.bestSolution[irace].selectedTune) {
              return false;
            }
            return true;
          })
    ...
    cgTryReplacer(round, _r, usedRids, usedHids, isolution, useGarage, rqSum, iround, notInUseOnly, isRerun, isOverRq) {
      let foundInUse;
      let foundInUseIndex;
      ...
      let currentRQ = _r.bestSolution[isolution] ? Vue.all_carsObj[_r.bestSolution[isolution].rid]?.rq || 0 : 0;

      let LIST = round.races[isolution].cars;
      if (isRerun) LIST = _r.solutions[isolution] || [];
      ...
      LIST.find(c => {
        ...
        mirrors.find(mirror => {
          ...
          if (mirror.usedIn === undefined && !foundNotInUse && (Vue.all_carsObj[mirror.rid].rq <= currentRQ || !isOverRq)) {
      ...
      if (foundNotInUse) {
        if (_r.bestSolution[isolution]?.rid) {
          ...
          if (useGarage) {
            ...
            let garageCar = Vue.garageByRid[_r.bestSolution[isolution].rid].find(x => this.isBetterTune(x, _r.bestSolution[isolution]) && usedHids[x.cardRecordId] === isolution);
    ...
    cgResolveSolutionsToUpgrade(_r, round, iround) {
      ...
      function pushSol(garageCar, car, irace, isBetter, isOp) {
        _r.solutions[irace].push({
          ...
          tune: car.tune,
          points: car.points,
          originalTune: (garageCar.tun || garageCar.tunZ),
          cardRecordId: garageCar.cardRecordId
        });
        if (isBetter) {
          delete _r.solutions[irace][_r.solutions[irace].length-1].originalTune;
          _r.solutions[irace][_r.solutions[irace].length-1].tune = (garageCar.tun || garageCar.tunZ);
        }
        if (isOp) {
          delete _r.solutions[irace][_r.solutions[irace].length-1].originalTune;
          _r.solutions[irace][_r.solutions[irace].length-1].tune = (garageCar.tun || garageCar.tunZ);
    ...
    cgDashToggleMyGarage(value) {
      if (value !== undefined) window.localStorage.setItem("cgDashUseMyGarage", JSON.stringify(value));
    ...
    cgReturnToSolutions(_r, round, useGarage, isolution) {
      if (!_r.solutions?.[isolution]) return;

      let found;
      let foundGarage;

      found = _r.solutions[isolution].find(x => {
        if (!x || !x.rid) return false;
        if (x.rid !== _r.bestSolution[isolution].rid) return false;
        if (_r.bestSolution[isolution].cardRecordId) { // tune
          if (x.tune !== _r.bestSolution[isolution].tune) return false;
        } else { // selectedTune
          if (x.tune !== _r.bestSolution[isolution].selectedTune) return false;
        }
        if (_r.bestSolution[isolution].cardRecordId && x.cardRecordId) {
          if (x.cardRecordId !== _r.bestSolution[isolution].cardRecordId) return false;
        }
        return true;
      });
      ...
      if (!found) {
        ...
        // }
        let obj = {
          rid: _r.bestSolution[isolution].rid,
        ...
        if (_r.bestSolution[isolution].cardRecordId) obj.cardRecordId = _r.bestSolution[isolution].cardRecordId;
        _r.solutions[isolution].push(obj);
    ...
    cgDashSolutionClick(car, iround, irace, ibank, e) {
      ...
      if (car && car.originalTune) {
        // upgrade simulation
        Vue.addToGarageUpgraded(car);
        delete car.originalTune;
        this.$store.commit("DEFINE_SNACK", {
          active: true,
          correct: true,
          text: this.$t("m_addedToGarageSim"),
          type: "success",
          time: 2000
        });
        this.cgReloadIndex = iround;
        ...
        // normal addreplace / free predict
        Vue.set(dashRound.bestSolution, irace, {
          ...JSON.parse(JSON.stringify(Vue.all_carsObj[car.rid])),
          selectedTune: car.tune,
          customData: {},
      ...
      if (backupSolutionCar) {
        ...
        dashRound.solutions[irace].push(solutionObj);
        this.cgSolutionsArrSort(dashRound.solutions[irace]);
      }
      
      this.cgResolveRqSums(dashRound);
    },
    cgSolutionsArrSort(arr) {
      arr.sort((a,b) => {
        if (Vue.all_carsObj[a.rid].rq === Vue.all_carsObj[b.rid].rq) {
          return a.rid.localeCompare(b.rid);
        }
    ...
    cgSolutionsSortByOrigTune(arr) {
      arr.sort((a,b) => {
        if (a.originalTune && !b.originalTune) return 1;
        if (!a.originalTune && b.originalTune) return -1;
        if (a.rid === b.rid) {
          if (a.originalTune !== b.originalTune) return b.originalTune.localeCompare(a.originalTune);
          return b.tune.localeCompare(a.tune);
        }
        return 0;
      });
    },
    cgResolveProblemRounds(rounds) {
      let problemRounds = [];
      rounds.map((round, iround) => {
        if (!this.cg.rounds[iround].lastAnalyze) return;
        let hasProblem = false;
        round.bestSolution.map((car, irace) => {
          if (!car || !car.rid) {
            hasProblem = true;
          }
        });
        if (!hasProblem) {
          if (round.bestSolutionRqSum > this.cg.rounds[iround].rqLimit) {
            hasProblem = true;
            round.solutions.map(race => {
              this.cgSolutionsArrSort(race);
            })
          }
        }
        if (hasProblem) {
          problemRounds.push(iround);
        }
      });
    ...
    cgNormalizeFormat(cg) {
      if (!cg.rounds) return;
      
      cg.rounds.map(round => {
        round.races.map(race => {
          if (race.tune && race.tune.includes("Other") && race.tune.length > 5) {
            race.tune = race.tune.replace("Other", "");
          }
          race.cars && race.cars.map((car, icar) => {
            if (Array.isArray(car) && typeof car[0] === "number") {
              Vue.set(race.cars, icar, {
                rid: Vue.rn_to_rid[car[0]],
                tune: car[1],
                selectedTune: car[1],
                points: car[2]
              });
    ...
    cgPrepareSaveRaw() {
      ...
      let parsed = JSON.parse(this.cgRaw);
      ...
      keys.map(key => {
        if (JSON.stringify(parsed[key]) !== JSON.stringify(this.cg[key])) params[key] = parsed[key];
      });
      ...
      this.cgSaveLoading = true;
      axios.post(Vue.preUrl + "/editCgRaw", params)
      .then(res => {
        this.cgSaveLoading = false;
    ...
    loadEventFull(date, e) {
      ...
      .then(res => {
        ...
        if (event.date === res.data.date) {
          ...
          if (this.user && this.user.mod) {
            this.eventRaw = JSON.stringify(res.data, null, 2);
          }
    ...
    eventStyleList() {
      ...
      this.eventList.sort((a,b) => {
        if (a.index === b.index && a.endDateTime && b.endDateTime) {
          ...
          if (!a.started && !b.started && b.startDateTime !== a.startDateTime) {
        ...
        dataTrack.map((car, icar) => {
          if (bestTime === undefined || (icar === this[key][itrack].icar)) {
            ...
            if (car.time <= 0) {
              ...
              car.timeToPrint = car.time.toTestBowlSrvDisplay();
            } else {
    ...
    eventAnalyseKFilter(forceFree) {
      ...
      let params = {
        ...
        myGarage: this.eventMyGarage && !!(this.whatTier && this.whatTier <= 3),
        picks: []
      }
      if (forceFree) params.myGarage = false;
    ...
    eventOpenShowCarDialog(car, e, igroup, icar) {
      ...
      this.tuneDialogCarRid = car.rid;
      car.selectedTune = car.tune;
      this.tuneDialogCarConfig = car;
      this.tuneDialogCarConfig.selectedTune = '000';
    ...
    eventPrepareSaveRaw() {
      let params = { date: this.eventCurrentId };
      let parsed = JSON.parse(this.eventRaw);
      ...
      let keys = [
        ...
        "startDateTime",
        "endDateTime",
        "ticketRegenerationTime",
        "prizes",
        "user"
      ]
      this.eventSortFlex(parsed);

      keys.map(key => {
        if (JSON.stringify(parsed[key]) !== JSON.stringify(this.event[key])) params[key] = parsed[key];
      });
    ...
    resolvePointsClassic() {
      ...
      result.map((x, ix) => {
        Object.keys( x ).forEach(function (trackId) {
          if (
            Vue.timesCache[vm.carDetailsList[ix].TCode + "_" + Vue.rn_to_track.indexOf(trackId)]
          ) {
            tempValue = Vue.timesCache[vm.carDetailsList[ix].TCode + "_" + Vue.rn_to_track.indexOf(trackId)];
            if (vm.carHoverIndex > -1 && vm.carHoverIndex !== ix) {
              let referenceTime = Vue.timesCache[vm.carDetailsList[vm.carHoverIndex].TCode + "_" + Vue.rn_to_track.indexOf(trackId)];
    ...
    kingAnalyseFinish() {
      ...
      .then(res => {
        ...
        // let result = [];
        res.data.map(car => {
          Vue.loadTimesFromKing(car, this.kingTrack.code);
          this.addCar({ rid: car.rid, selectedTune: car.tune });
        })
        // Vue.set(this, "carDetailsList", result);
        // this.applyNewData(res.data, this.mode === 'challenges', origMode);
        // if (this.eventMyGarage) {
    ...
    createNewCgByJson(e) { // cg old
      if (e) {
        ...
        if (text.includes('zoneEligibility') && text.includes('zoneSize')) { // complete cg rounds
          if (this.mode === 'challenges') {
            this.cgCompleteJson = text;
            this.cgSubmitCompleteJson(true);
            return;
          }
        }
    ...
    openCarTuneDialog(ev) {
      let car = ev.car;
      if (car) {
        this.tuneDialogCarRid = car.rid;
        this.tuneDialogCarConfig = car;
        this.tuneDialogCarIndex = ev.index;
        this.tuneDialogActive = true;
        this.tuneDialogTuneWins = [];
        this.tuneDialogTuneLose = [];
        this.tuneDialogisOppo = false;
    ...
    mainCarsListGeneralClick(e) {
      ...
      if (code) {
        let carIndex = code.split("_")[0];
        let trackIndex = code.split("_")[1];
        let car = this.carDetailsList[carIndex];
